...
```{r child = file.path("fragments", "description.qmd")}
```

## Prerequisites

```{r child = file.path("fragments", "prerequisites.qmd")}
```

# Learning Objectives

```{r child = file.path("fragments", "learning-objectives.qmd")}
```
...
Go to the [Course Software](ref-course-software.qmd) page for detailed instructions on how to install the software we'll be using. Here's a quick list:
...
This class can be challenging - don't suffer in silence. Look at the ["Getting Help"](ref-getting-help.qmd) page for ways to get resources that can help you succeed.
